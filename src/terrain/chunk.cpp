...
Chunk::Chunk(size_t x, size_t y, const TexManager& terrainTexManager, const Map& map) :
	_chunkPos(x,y),
	_visible(false),
...
std::vector<std::vector<Biome> > Chunk::getMapInfo() {
...
void Chunk::addPointToTransitionData(Biome biome, size_t i, size_t j, float distance) {
...
void Chunk::correctDistance(Biome biome, size_t k, float distance) {
...
std::vector<std::vector<bool> > Chunk::computeTransitionData() {
	size_t size1 = _size - 1;
	float step =  CHUNK_SIZE / (float) size1;
...
void Chunk::computeIndices(const std::vector<std::vector<Biome> >& biomes,
...
void Chunk::generateBuffersPlain() {
...
void Chunk::generateBuffersTransition() {
	for (auto it = _transitionData.begin(); it != _transitionData.end(); it++) {
...
void Chunk::computeLowestsHighests() {
	size_t size1 = _size-1;
	float step =  CHUNK_SIZE / (float) size1;
...
void Chunk::drawTransitions() const {
	for (auto it = _transitionData.begin(); it != _transitionData.end(); it++) {
		_terrainTexManager.bindTexture(it->first);
...
float Chunk::getHeight(float x, float y) const {
