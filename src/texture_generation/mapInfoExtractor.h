...
public:
  ...
  GeneratedImage imageFusion(const std::array<const GeneratedImage*, (int) Biome::BIOME_NB_ITEMS>& imgs) const;

  inline size_t getSize() const {return _size;}
  inline float getTransitionSize() const {return _transitionSize;}
  inline const GeneratedImage& getBiomeMask(Biome biome) const {return _biomesMasks[(int) biome];}
...
private:
  ...
  std::array<GeneratedImage, (int) Biome::BIOME_NB_ITEMS> _biomesMasks;
